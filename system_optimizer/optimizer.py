...
service_logger = get_logger("Service Management")
optimizer_logger = get_logger("System Tuning")
...
class ServiceManager:
    ...
    def list_services(self) -> List[Dict[str, str]]:
        if not SYSTEMD_PATH:
            service_logger.error("systemctl is not available on this system")
            return []
        try:
            result = subprocess.run(
                [SYSTEMD_PATH, "list-units", "--type=service", "--all", "--no-pager", "--plain", "--no-legend"],
                check=True,
                capture_output=True,
                text=True,
            )
        except subprocess.CalledProcessError as exc:
            service_logger.error("Failed to list services: %s", exc)
            return []
    ...
    def _run_action(self, service: str, action: str) -> bool:
        if not SYSTEMD_PATH:
            service_logger.error("systemctl is not available for %s action", action)
            return False
        try:
